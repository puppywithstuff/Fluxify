...
if (!window.__asteroidsBookmarkletRunning) {
    ...
    .ast-stop-btn { font-family: Orbitron, Impact, sans-serif; }
    .ast-levelup-text {
        position: fixed;
        top: 45%;
        left: 50%;
    ...
    .ast-class-overlay {
        ...
        border: 2px solid rgba(0,255,221,0.14);
        box-shadow: 0 0 40px rgba(0,255,221,0.06);
        padding: 20px;
        border-radius: 12px;
        z-index: 2147483650;
        min-width: 320px;
    ...
    .ast-class-overlay h3 { margin:0 0 12px; color:#0ff; letter-spacing:1px; }
    .ast-class-btn {
        width: 100%;
        margin: 8px 0;
        padding: 12px;
        ...
        font-weight: 700;
        font-size: 16px;
        box-shadow: 0 0 20px rgba(0,255,221,0.06);
    }
    .ast-class-btn.full { background: linear-gradient(90deg,#032CDE,#00FFB2); color:#001; }
    .ast-class-btn.sharp { background: linear-gradient(90deg,#440022,#FF4466); color:#fff; }
    .ast-class-btn.scout { background: linear-gradient(90deg,#112233,#88FFEE); color:#001; }
    `;
    ...
    function initGame() {
        ...
        ship = {
            ...
            immunityBlink: 0,
            shootCooldown: 0,
            // class properties (default, will be set by overlay)
            playerClass: null,
            autoFire: false,
            showLaser: false,
            modSlot: false
        ...
        // Show class overlay right after ship spawn
        createClassOverlay();
    }
    ...
    function shoot(){
        ...
        bullets.push({x:bx, y:by, vx:ship.vx+Math.cos(ship.a)*speed, vy:ship.vy+Math.sin(ship.a)*speed, life:50, damage: upgrades.damage, pierce: upgrades.pierce });
    }
    ...
    function updateBossHealthBar() {
        const healthBar = document.querySelector('.ast-boss-health');
        const innerBar = document.querySelector('.ast-boss-health-inner');
        if (!boss || !healthBar || !innerBar || gameOver || paused) {
            removeBossHealthBar();
            return;
        }
        innerBar.style.width = (boss.health / boss.maxHealth * 100) + '%';
    }
    ...
    function explodeBoss() {
        ...
        removeBossHealthBar();
        upgradeReady = true;
        setTimeout(() => nextLevelBeforeLevelUp(), 500);
        setTimeout(() => showLevelUp(level+1), 1200);
        setTimeout(() => showUpgrade(), 2800);
    }

    // Reset level state before level up text fades in
    function nextLevelBeforeLevelUp() {
        level++;
        bossDeathAnim = null;
        boss = null;
        bossLasers = [];
    ...
    function showUpgrade() {
        ...
        upgradesList.forEach(opt => {
            ...
            btn.onclick = () => {
                upgrades[opt.key]++;
                upgradeModal.innerHTML = `<div class="ast-upgrade-confirm">Upgrade: ${opt.label}!<br>Ready for LEVEL ${level}</div>`;
                setTimeout(() => {
                    upgradeModal.style.display = 'none';
    ...
    function updateBoss(dt) {
        ...
        for (let i = bullets.length - 1; i >= 0; i--) {
            ...
            if (b.x > boss.x - boss.w/2 && b.x < boss.x + boss.w/2 &&
                ...
                // If bullet pierces, reduce health by bullet.damage but keep bullet
                boss.health -= b.damage;
                spawnParticle(b.x, b.y);
                if (upgrades.pierce > 0 && b.pierce > 0) {
                    b.pierce--;
                } else {
                    bullets.splice(i, 1);
                }
    ...
    function shootAtPlayer(e){ const angle=Math.atan2(ship.y-e.y,ship.x-e.x); const speed=6; enemyBullets.push({x:e.x+Math.cos(e.a)*e.r,y:e.y+Math.sin(e.a)*e.r,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,life:120, damage: 1}); }
    function updateEnemies(dt){
        ...
        // Enemy bullets collide with asteroids and player bullets
    ...
    function update(dt){
        if(gameOver || paused) return; // PAUSE GAME during modal
        if(ship.immunity > 0) {
            ship.immunity -= dt;
        ...
        // --- firing logic updated to support Full Auto ---
        if(keys[' ']){
            if(ship.autoFire){ shoot(); }
            else if(!keys._spaceDown){ shoot(); keys._spaceDown=true; }
        }
        ...
        for(let i=asteroids.length-1;i>=0;i--){
            ...
            for(let j=bullets.length-1;j>=0;j--){ 
                ...
                if(dist(b,a)<a.r+7){ 
                    ...
                    asteroids.splice(i,1); score+=100; 
                    for(let k=0;k<Math.floor(a.r/2);k++) spawnParticle(a.x,a.y); 
    ...
    function render(){
        ...
        if(!gameOver){
            ...
            if(drawShip){
                ...
                // Sharp Shooter laser sight (drawn in ship-local coords)
                if(ship.showLaser){
                    ctx.save();
                    ctx.globalAlpha = 0.9;
                    ctx.lineWidth = 2;
                    ...
                    // draw until edge of screen in current facing direction
                    const maxLen = Math.max(W,H) * 2;
                    ctx.lineTo(maxLen, 0);
                    ctx.stroke();
                    // far dot
                    ctx.beginPath();
                    ctx.arc(maxLen,0,3,0,Math.PI*2);
    ...
    function restartGame(){ level=1; upgrades={fireRate:1,bulletSpeed:1,shipSpeed:1,damage:1,pierce:0}; initGame(); }
    function cleanup(){
        window.__asteroidsBookmarkletRunning=false;
        clearInterval(stopBtnInterval);
        try { overlay.remove(); } catch(e){}
        try { root.remove(); } catch(e){}
        try { fireBtn.remove(); } catch(e){}
        try { stopBtn.remove(); } catch(e){}
        try { style.remove(); } catch(e){}
        try { gameOverText.remove(); } catch(e){}
        try { restartBtn.remove(); } catch(e){}
        try { levelUpText.remove(); } catch(e){}
        try { upgradeModal.remove(); } catch(e){}
        removeBossHealthBar();
    }
    ...
    // === CLASS OVERLAY CREATION FUNCTION ===
    function createClassOverlay(){
        // If overlay already exists, don't recreate
        if(document.querySelector('.ast-class-overlay')) return;

        const ov = document.createElement('div');
        ov.className = 'ast-class-overlay';
        ov.innerHTML = '<h3>SELECT YOUR CLASS</h3><div style="font-size:12px;opacity:0.8;margin-bottom:10px">Choose the ship class to start with.</div>';

        function makeBtn(text, cls, onClick){
            const b = document.createElement('button');
            b.className = 'ast-class-btn ' + cls;
            b.textContent = text;
            b.addEventListener('click', () => {
                onClick();
                ov.remove();
                // give small immunity so player doesn't die immediately
                ship.immunity = 60;
            });
            return b;
        }

        ov.appendChild(makeBtn('Full Auto — Hold to spray', 'full', ()=>{
            ship.playerClass = 'fullauto';
            ship.autoFire = true;
            ship.showLaser = false;
            ship.modSlot = false;
            upgrades.fireRate = 2.5;
            upgrades.damage = 0.7;
            upgrades.bulletSpeed = 1;
            upgrades.pierce = 0;
            upgrades.shipSpeed = 1;
        }));

        ov.appendChild(makeBtn('Sharp Shooter — Laser sight', 'sharp', ()=>{
            ship.playerClass = 'sharp';
            ship.autoFire = false;
            ship.showLaser = true;
            ship.modSlot = false;
            upgrades.fireRate = 0.6;
            upgrades.damage = 3.0;
            upgrades.bulletSpeed = 2.0;
            upgrades.pierce = 0;
            upgrades.shipSpeed = 1;
        }));

        ov.appendChild(makeBtn('Scout — Fast & Pierce', 'scout', ()=>{
            ship.playerClass = 'scout';
            ship.autoFire = false;
            ship.showLaser = false;
            ship.modSlot = true;
            upgrades.fireRate = 1.4;
            upgrades.damage = 0.9;
            upgrades.bulletSpeed = 1.2;
            upgrades.pierce = 2;
            upgrades.shipSpeed = 1.3;
        }));
